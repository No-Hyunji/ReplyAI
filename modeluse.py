from pymongo import MongoClientt tensorflow as tf
import numpy as np
from konlpy.tag import Okt
from p
imporprint import pprint
...
def mongo_select_all():
    for one in collection.find({'movieNm': {'$eq': '야구소녀'}}, {'_id':0, 'movieNm':1, 'content':1,'score':1}): #제목,
...
def predict_pos_neg(review):
    ...
    if (score > 0.5):
        pos_count += 1
        print("[{}]는 {:.2f}%의 확률로 긍정리뷰이지 않을까 추측해봅니다^^\n".format(review, score * 100))
    else:
